"""Deploy: Logística/distribución + capacidad de producción + Excel mejorado (Junio-24)"""
import paramiko, os
...
print('\n=== Tinker: entrega_domicilio = false para proveedores ya aprobados ===')
run(ssh, f'cd {RROOT} && /usr/bin/php artisan tinker --execute="App\\\\Models\\\\Restaurantero::where(\'perfil_completo\',true)->whereNull(\'entrega_domicilio\')->update([\'entrega_domicilio\' => false]);" 2>&1')

print('\n=== Optimizar Laravel ===')
run(ssh, f'cd {RROOT} && /usr/bin/php artisan view:clear 2>&1')
run(ssh, f'cd {RROOT} && /usr/bin/php artisan config:cache 2>&1')
run(ssh, f'cd {RROOT} && /usr/bin/php artisan route:cache 2>&1')
