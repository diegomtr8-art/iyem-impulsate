"""Deploy 2026-06-06: Auditoría completa — eliminación azules, seguridad, dark mode, móvil"""
import paramiko, os
...
print('\n=== PHP — Seguridad y lógica ===')
up(sftp, 'app/Http/Middleware/HandleInertiaRequests.php')
up(sftp, 'app/Http/Controllers/RestauranteroCitasController.php')
...
print('\n=== Artisan optimize en servidor ===')
run(ssh, 'cd ' + RROOT + ' && php artisan config:clear && php artisan view:clear && php artisan optimize 2>&1')
run(ssh, 'cd ' + RROOT + ' && php artisan route:cache 2>&1')
